        ...
        <template v-for="item in orders" :key="item.id">
          <tr v-if="orders.length" :class="{ 'text-secondary': !item.is_paid }">
            ...
            <td><span v-text="item.user.email" v-if="item.user" class="d-inline-block text-truncate"
                style="max-width: 12.5rem"></span></td>
            <td>
              <ul class="list-unstyled">
                <template v-for="(product, key, index) in item.products" :key="product.product.id">
                  <li v-if="index < 3">
                    <small> {{ product.product.title }} |{{ index }}數量：{{ product.qty }} {{ product.product.unit
                      }}</small>
                  </li>
                </template>

                <li v-if="Object.keys(item.products).length > 3">
                  <a href="#" @click.prevent="openModal(item)" class="nav-link"> <small> ...More</small> </a>
                </li>
              </ul>
            </td>
